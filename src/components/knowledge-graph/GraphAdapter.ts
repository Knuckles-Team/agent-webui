...
export const knowledgeGraphToGraphology = (
  ...
  const structuralNodes = nodes.filter(n => n.labels.some(l => ['KnowledgeBase', 'Project', 'Folder', 'User'].includes(l)));
  const otherNodes = nodes.filter(n => !n.labels.some(l => ['KnowledgeBase', 'Project', 'Folder', 'User'].includes(l)));
