        ...
        <Card className="border-border/40 backdrop-blur-md bg-card/65 relative overflow-hidden shadow-md">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Objective Target</label>
              <Textarea
            ...
            {showConfig && (
              <div className="space-y-4 pt-2 border-t border-border/20 animate-in fade-in slide-in-from-top-1 duration-200">
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Guardrail Constraints</label>
                  ...
                  {constraints.length > 0 && (
                    <div className="p-2 border border-border/20 rounded bg-muted/20 max-h-[120px] overflow-y-auto space-y-1">
                      {constraints.map((c, i) => (
                        <div key={i} className="flex justify-between items-center text-[10px] bg-background/50 px-2 py-1 rounded border border-border/10">
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs font-medium line-clamp-1 flex-1 pr-1">
                      {g.iterations[0]?.action?.replace("Analyzing workspace and executing step 1 for objective: '", "")?.replace("'.", "") || "Objective Execution"}
                    </span>
                    <Badge
                      className={cn(
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-muted-foreground/80 font-mono">
                    <span className="flex items-center gap-1"><RotateCcw className="size-3" /> Step {g.total_iterations}</span>
                    <span className="flex items-center gap-1"><Clock className="size-3" /> {(g.total_duration_ms / 1000).toFixed(1)}s</span>
                    <span className="flex items-center gap-1"><Terminal className="size-3" /> {g.total_tool_calls} tools</span>
                  </div>
                </div>
              ))
            )}
                    ...
                    <div key={step.iteration} className="relative pl-6">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 text-[10px] font-mono text-muted-foreground/60 shrink-0">
                          ...
                          {expandedSteps[step.iteration] ? <ChevronUp className="size-3.5" /> : <ChevronDown className="size-3.5" />}
                        </div>
                      </div>
                      ...
                      {expandedSteps[step.iteration] && (
                        <div className="mt-3 pl-2 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
                          ...
                          <div className="text-xs text-[#d1d5db] bg-muted/15 border border-border/10 rounded-lg p-3 leading-relaxed">
                            <span className="font-semibold text-primary/80 font-mono block text-[10px] uppercase tracking-wider mb-1">Observation</span>
                            {step.result}
                          </div>
                          ...
                          {step.validation_output && (
                            <div className="rounded-lg overflow-hidden border border-border/10 bg-black font-mono">
                              <div className="bg-muted/10 border-b border-border/10 px-3 py-1.5 flex items-center gap-2 justify-between text-[9px] text-muted-foreground">
                                <span className="flex items-center gap-1.5"><Terminal className="size-3" /> CLI Verifier Console</span>
                                <span>Output</span>
                              </div>
          ...
          <Card className="border-border/40 bg-muted/5 border-dashed min-h-[600px] flex items-center justify-center text-center">
            <CardContent className="flex flex-col items-center justify-center p-12">
              ...
              <p className="text-muted-foreground text-sm max-w-sm mt-1">
                Select an active goal execution run from the left panel registry or launch a new autonomous loop to track live progression here.
              </p>
            </CardContent>
          </Card>
        )}
