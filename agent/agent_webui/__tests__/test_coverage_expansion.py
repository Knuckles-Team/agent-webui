...
def client():
    ...
    # Create a dummy agent and workspace helpers
    mock_agent = Agent('google-gla:gemini-1.5-flash')
...
def test_link_nodes_success(client, mock_graph_engine):
    with patch('agent_webui.api_extensions.IntelligenceGraphEngine.get_active', return_value=mock_graph_engine):
        link_data = {'source': 'node1', 'target': 'node2', 'type': 'RELATED_TO'}
...
def test_list_resources_success(client, mock_graph_engine):
    with patch('agent_webui.api_extensions.IntelligenceGraphEngine.get_active', return_value=mock_graph_engine):
        mock_graph_engine.backend.execute.return_value = [{'r': {'id': 'res1', 'name': 'Resource 1'}}]
        response = client.get('/api/enhanced/graph/resources')
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
...
def test_execute_cypher_error(client, mock_graph_engine):
    with patch('agent_webui.api_extensions.IntelligenceGraphEngine.get_active', return_value=mock_graph_engine):
        ...
        response = client.post('/api/enhanced/graph/query', json={'query': 'MATCH (n) RETURN n'})
        assert response.status_code == 500
...
def test_kb_health_check_error(client, mock_graph_engine):
    ...
    mock_kb_engine.health_check = AsyncMock(side_effect=Exception('Health check failed'))
