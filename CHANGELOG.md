...
### Added
- **Governed contact delivery API** — `agent_webui.contact_delivery` defines the
  bounded delivery contract, and `create_agent_web_app(..., contact_delivery=...)`
  wires an authenticated, same-origin `POST /api/contact` route that fails closed
  when its host-provided adapter or server policy is unavailable.
